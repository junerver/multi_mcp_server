...
def get_system_resource() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "contents": [{
                "type": "text",
                "text": "\n".join(ALLOWED_DIRECTORIES) if ALLOWED_DIRECTORIES else "No allowed directories configured"
            }]
        }
        ...
        return {
            "contents": [{
                "type": "text",
                "text": f"Error accessing resource: {e}"
            }]
        }
...
@mcp.tool()
def read_text_file(request: FileReadRequest) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        with open(validated_path, "r", encoding=request.encoding) as f:
            content = f.read()
            
            # Handle head/tail parameters
            if request.head > 0:
                lines = content.splitlines()
                content = '\n'.join(lines[:request.head])
            elif request.tail > 0:
                lines = content.splitlines()
        ...
        return {
            "content": [{
                "type": "text",
                "text": f"File not found: {request.path}"
            }]
        }
        ...
        return {
            "content": [{
                "type": "text",
                "text": f"Permission denied: {request.path}"
            }]
        }
    except Exception as e:
        return {
...
class ReadMediaFileRequest(BaseModel):
    path: str = Field(..., description="要读取的媒体文件路径")

@mcp.tool()
def read_media_file(request: ReadMediaFileRequest) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            "content": [{
                "type": "text",
                "text": f"Error reading media file: {e}"
            }]
        }
...
def write_file(request: FileWriteRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        with open(validated_path, "w", encoding=request.encoding) as f:
            f.write(request.content)
        return {
            "content": [{
                "type": "text",
                "text": f"File written successfully: {request.path}"
            }]
        }
    except Exception as e:
        return {
...
def create_directory(request: CreateDirectoryRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "content": [{
                "type": "text",
                "text": f"Directory created successfully: {request.path}"
            }]
        }
    except Exception as e:
        return {
            "content": [{
                "type": "text",
                "text": f"Error creating directory: {e}"
            }]
        }
    ...
    try:
        ...
        if not os.path.isdir(validated_path):
            return {
                "content": [{
                    "type": "text",
                    "text": f"Error: '{request.path}' is not a directory"
                }]
            }
...
@mcp.tool()
def read_multiple_files(request: ReadMultipleFilesRequest) -> Dict[str, Any]:
    """
    同时读取多个文件。
    失败的读取不会停止整个操作。
    """
...
@mcp.tool()
def move_file(request: MoveFileRequest) -> Dict[str, Any]:
    """
    移动或重命名文件和目录。
    如果目标已存在则失败。
    """
    try:
        ...
        validated_destination = validate_path(request.destination)
        
        if not os.path.exists(validated_source):
            return {
                "content": [{
                    "type": "text",
                    "text": f"Error: Source '{request.source}' does not exist"
                }]
            }
        ...
        if os.path.exists(validated_destination):
            return {
                "content": [{
                    "type": "text",
                    "text": f"Error: Destination '{request.destination}' already exists"
                }]
            }
        ...
        return {
            "content": [{
                "type": "text",
                "text": f"Successfully moved '{request.source}' to '{request.destination}'"
            }]
        }
    except Exception as e:
        return {
...
class SearchFilesRequest(BaseModel):
    ...
    exclude_patterns: Optional[List[str]] = Field(default=[], description="要排除的模式列表")
...
def search_files(request: SearchFilesRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        matches = []
        pattern_lower = request.pattern.lower()
        exclude_patterns_lower = [p.lower() for p in (request.exclude_patterns or [])]
        ...
        if not matches:
            return {
                "content": [{
                    "type": "text",
                    "text": f"No files or directories found matching pattern '{request.pattern}'"
                }]
            }
...
class GetFileInfoRequest(BaseModel):
    path: str = Field(..., description="要获取信息的文件或目录路径")

@mcp.tool()
def get_file_info(request: GetFileInfoRequest) -> Dict[str, Any]:
    """
    获取文件或目录的详细元数据。
    ...
    try:
        ...
        if not os.path.exists(validated_path):
            return {
                "content": [{
                    "type": "text",
                    "text": f"Error: Path '{request.path}' does not exist"
                }]
            }
    ...
    try:
        ...
        if not os.path.exists(validated_path):
            return {
                "content": [{
                    "type": "text",
                    "text": f"Error: File '{request.path}' does not exist"
                }]
            }
        ...
        # 应用编辑操作
        for i, edit_op in enumerate(request.edits):
            ...
            if old_text in modified_content:
                ...
                if old_lines and new_lines:
                    ...
                    if original_indent:
                        # 应用相同的缩进到新文本
                        new_lines = [original_indent + line.lstrip() if line.strip() else line 
        ...
        # 生成差异
        diff_output = _generate_diff(original_content, modified_content, request.path)
        ...
        if request.dry_run:
            ...
            result_text = f"DRY RUN - File: {request.path}\n"
            ...
            result_text = f"File edited successfully: {request.path}\n"
            result_text += f"Indent style: {indent_style}\n\n"
